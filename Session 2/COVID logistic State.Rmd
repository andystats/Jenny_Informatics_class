...
# Update: stuff keeps breaking - trust nothing :)
...
### Updated to double logistic, but harvesting model parameters broken/wonky
...
dygraph(cases.ts, main = paste("Daily increase COVID cases in", State)) %>%
  ...
  dyShading(from = "2020-11-03", to = Sys.Date(), color = "#FFE6E6") %>%
  dyRangeSelector()
...
```{r}
plot(x,y, ylim=c(0,700000), xlim=c(50,620), ylab = "Cumulative case count", xlab = "days since first case", axes=F); axis(1); axis(2)
newx  <- seq(50,500)
